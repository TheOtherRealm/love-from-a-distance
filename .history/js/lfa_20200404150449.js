...
(function () {
	...
	Handlebars.registerHelper('input-tel', function(c,o){
		console.log(c,';',o);
		var html='<p><label for="'+c.id+'">'+c.label+(c.required?'<span class="gray">*</span>:':':')+'</label><br>\
		<input type="text" id="'+c.id+'" name="'+c.id+'" pattern="([0-9]{3}-[0-9]{3}-[0-9]{3}-[0-9]{4}|[0-9]{3}-[0-9]{3}-[0-9]{4}|[0-9]{11}|[0-9]{10})" placeholder="'+c.placeholder+'" "'+c.required+'" class="form-control"></p>'
		return html;
	});
