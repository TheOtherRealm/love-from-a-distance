...
(function () {
	...
	Handlebars.registerHelper('input-text', function(c,o){
		...
		<input type="text" id="'+c.id+'" name="'+c.id+'" placeholder="'+c.text+'" required class="form-control"></p>'
		return html;
	});
	Handlebars.registerHelper('input-email', function(c,o){
		console.log(c,';',o);
		var html='<p><label for="'+c.id+'">'+c.label+'</label><br>\
		<input type="text" id="'+c.id+'" name="'+c.id+'" placeholder="'+c.placeholder+'" required class="form-control"></p>'
		return html;
	});
	var source = $('#app').html();
