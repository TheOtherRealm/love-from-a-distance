...
Vue.component('be', {
	props: ['tobe'],
	template: '<p>Would you like to be <span class="bold">yourself</span>, a <span class='bold'>predesigned</span> fictional	character<span class="gray superscript" title="(Potentially costs money)">$</span>, a fictional character<span class="bold">you create</span>, or <span class="bold">multiple</span> characters<span class='gray superscript' title="(Potentially costs money)">$</span>?<br><input type="radio" :id="tobe.id" name="be" :value="tobe.id" required> <label :for="tobe.id">{{tobe.text}}</label></div>'
});
var app = new Vue({
	el: '#app',
	data: {
