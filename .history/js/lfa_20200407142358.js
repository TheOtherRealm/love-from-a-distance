	...
	$(document).ready(function () {
		$('form').submit(function (f) {
			...
			$('#' + form.attr('id') + ' input, #' + form.attr('id') + ' select, #' + form.attr('id') + ' textarea').each(function (i, e) {
				console.log(JSON.parse($(this).serializeArray().push($(this).data("type"))),$(this).data("type"),serialized[i]);
				serialized[i]=JSON.parse($(this).serializeArray().push($(this).data("type")));
			})
			// .then(function(){
