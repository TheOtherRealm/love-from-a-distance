...
(function () {
	...
	Handlebars.registerHelper('select-multiple', function(c,o){
		console.log(c,';',o);
		var html='<label>{{{you.waysto.label}}}</label>\
		<select name="waysto" multiple class="form-control">\
			{{#each you.waysto.option}}\
			<option value="{{this.id}}" id="{{this.id}}" class="form-control">{{{this.text}}}</option>\
			{{/each}}\
		</select>'
		return html;
	});
